...
SELECT CLIENT, TITLE, TO_NUMBER(DURATION), TO_NUMBER(SEASON), TO_NUMBER(EPISODE), TO_DATE(VIEWDATE || ' ' || VIEWHOUR, 'YYYY-MM-DD HH24:MI'), TO_NUMBER(SUBSTR(VIEWPCT, 1, INSTR(VIEWPCT, '%')-1), '999') FROM FSDB.OLD_TAPS JOIN TVSERIES ON TITLE_SERIES = TITLE WHERE SEASON IS NOT NULL;
