...
IS
  ...
  IS
    ...
    FROM
      ...
      LEFT OUTER JOIN LIC_SERIES ON SERIES_TITLE = TITLE AND CLIENTID = CLIENT AND SERIES_EPISODE = EPISODE AND SEASON_SERIES = SEASON;
  ...
  BEGIN
    ...
    LOOP
      IF REMAINDER(ROUND(MONTHS_BETWEEN(TO_DATE(PERIOD_TO_BILL, 'MON-YYYY'), CLIENTID.STARTDATE), 0), 8) = 0
      THEN
