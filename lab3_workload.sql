...
LOOP
  INSERT INTO TAPS_MOVIES VALUES(X.CONTRACTID, SYSDATE-(3/24), 91, 'Avatar');
  INSERT INTO TAPS_SERIES VALUES(X.CONTRACTID, SYSDATE-(3/24), 91, 'House', 1, 1);
  INSERT INTO TAPS_SERIES VALUES(X.CONTRACTID, SYSDATE-(3/24), 91, 'Sherlock', 1, 1);
  INSERT INTO TAPS_SERIES VALUES(X.CONTRACTID, SYSDATE-(3/24), 91, 'Columbo', 1, 1);
END LOOP;
...
FOR X IN active_contracts
LOOP
  DELETE FROM TAPS_MOVIES WHERE CONTRACTID=X.CONTRACTID AND TO_CHAR(VIEW_DATETIME, 'DD-MM-YYYY HH')=TO_CHAR(SYSDATE-(3/24), 'DD-MM-YYYY HH') AND PCT=91 AND TITLE='Avatar';
  DELETE FROM TAPS_SERIES WHERE CONTRACTID=X.CONTRACTID AND TO_CHAR(VIEW_DATETIME, 'DD-MM-YYYY HH')=TO_CHAR(SYSDATE-(3/24), 'DD-MM-YYYY HH') AND PCT=91 AND TITLE='House' AND SEASON=1 AND EPISODE=1;
  DELETE FROM TAPS_SERIES WHERE CONTRACTID=X.CONTRACTID AND TO_CHAR(VIEW_DATETIME, 'DD-MM-YYYY HH')=TO_CHAR(SYSDATE-(3/24), 'DD-MM-YYYY HH') AND PCT=91 AND TITLE='Sherlock' AND SEASON=1 AND EPISODE=1;
  DELETE FROM TAPS_SERIES WHERE CONTRACTID=X.CONTRACTID AND TO_CHAR(VIEW_DATETIME, 'DD-MM-YYYY HH')=TO_CHAR(SYSDATE-(3/24), 'DD-MM-YYYY HH') AND PCT=91 AND TITLE='Columbo' AND SEASON=1 AND EPISODE=1;
END LOOP;
